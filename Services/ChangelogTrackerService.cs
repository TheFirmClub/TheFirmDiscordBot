...
{
    private const int AutoPostDebounceSeconds = 60;
    private const bool BackfillOnStartup = true;
    private const int BackfillMaxMessages = 10000;
    ...
    private readonly ConcurrentDictionary<ulong, int> _messageItemCounts = new();
    private readonly ConcurrentDictionary<ulong, ulong> _messageAuthors = new();
    ...
    private ulong? _lastStatsMessageId;
    ...
    private async Task OnReady()
    {
        Load();
        Console.WriteLine(
            $"ChangelogTrackerService ready. Last posted stats message id: {_lastStatsMessageId?.ToString() ?? "<none>"}");
    ...
    private async Task BackfillHistoryAsync()
    {
        ...
        {
            Console.WriteLine($"[ChangelogTracker] Source channel {SourceChannelId} not found.");
            return;
        }
...
// ✅ ONLY CHANGED SECTION IS HERE
    private async Task PostStatsEmbed()
    {
        ...
        if (guild != null)
        {
            foreach (var u in _byUser.Values)
            {
                var member = guild.GetUser(u.UserId);
                if (member == null) continue;

                if (member.Roles.Any(r =>
                        r.Id == 1407655775077404806UL ||
                        r.Id == 1393733280523882546UL ||
                        r.Id == 1421177514189127840UL ||
                        r.Id == 1393590761953558608UL))
                {
                    users.Add(u);
                }
            }
        ...
        {
            users = _byUser.Values.ToList();
        }

        int totalItems = users.Sum(s => s.Items);
        ...
        var embed = new EmbedBuilder()
            ...
            .WithDescription(desc.Length > 0 ? desc.ToString() : "No data yet.")
    ...
    {
        var stats = _byUser.GetOrAdd(userId, _ => new UserStats { UserId = userId });
    ...
    private void ScheduleDebouncedPost()
    {
        if (_debounceTimer == null)
        {
            ...
            _debounceTimer.Elapsed += async (_, __) => await PostStatsEmbed();
        }
        else
        {
            _debounceTimer.Stop();
    ...
    public class PersistedState
    {
        ...
        public Dictionary<ulong, UserStats> Users { get; set; } = new();
    }
}